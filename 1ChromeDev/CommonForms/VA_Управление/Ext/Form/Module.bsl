...
Процедура ВыполнитьТест(Команда)
	...
	КомандаJSON = "{""id"":1,""method"":""Runtime.enable""}";
...
Процедура Демонстрация(Команда)
	...
	ТекстJavaScript = ПолучитьТекстМакета("VA_LeaderLine");
	ПодключитьJavaScript(ТекстJavaScript);
	Возврат;
...
&НаКлиенте
Процедура ПодключитьJavaScript(ТекстJavaScript)
	
	ВременныйФайл = ПолучитьИмяВременногоФайла();
	
	Массив = Новый Массив;
	Массив.Добавить(ТекстJavaScript);
	ТекстJSON = ЗаписатьОбъектJSON(Массив);
	ТекстJSON = "S(){eval.apply(null," + ТекстJSON + ")";
	ЗаписьТекста = Новый ЗаписьТекста(ВременныйФайл, КодировкаТекста.UTF8);
	ЗаписьТекста.Записать(ТекстJSON);
	ЗаписьТекста.Закрыть();
	
	ПараметрыМетода = Новый Структура("expression,sourceURL,persistScript", "S()", ВременныйФайл, Истина);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПодключенJavaScript", ЭтотОбъект);
	НачатьВызовВебСокет(ОписаниеОповещения, "Runtime.compileScript", ПараметрыМетода);
	
КонецПроцедуры

&НаКлиенте
Процедура ПодключенJavaScript(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ДанныеJSON =  ПрочитатьСтрокуJSON(РезультатВызова);
	ПараметрыМетода = Новый Структура("scriptId", ДанныеJSON.ScriptId);
	НачатьВызовВебСокет(Новый ОписаниеОповещения, "Runtime.runScript", ПараметрыМетода);
