...
Процедура ПолученаВерсияКомпоненты(Значение, ДополнительныеПараметры) Экспорт
	...
	Заголовок = "Управление браузером, версия компоненты " + Значение;
...
Процедура ЗапуститьКлиентТестирования(Команда)
	...
	HTTPЗапрос = Новый HTTPЗапрос("/json/new?" + ПолучитьСтрокуЗапуска(АдресПубликации, ИдентификаторКлиента));
...
Процедура ВыполнитьJavaScript(ТекстJavaScript)
	...
	ВнешняяКомпонента.ВебСокет(АдресВебСокет, ЗаписатьОбъектJSON(ДанныеJSON));
