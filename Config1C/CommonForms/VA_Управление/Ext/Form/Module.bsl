...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПортБраузера = 9222;
	АдресПубликации = "http://localhost/test83/";
...
Процедура ЗапуститьКлиентТестирования(Команда)
	...
	Для каждого ЭлементJSON из ДанныеJSON Цикл
		Если ЭлементJSON.url = "about:blank" Тогда
			АдресВебСокет = ЭлементJSON.webSocketDebuggerUrl;
...
Процедура ИерархияЭлементовПриАктивизацииСтроки(Элемент)
	...
	ВнешняяКомпонента.НачатьВызовВебСокет(Новый ОписаниеОповещения, АдресВебСокет, ТекстJSON);
