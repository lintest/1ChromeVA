...
Процедура ПодключениеОбработкиПродолжение(ИмяОбработки)
	ТестируемаяБиблиотека = ПолучитьЗначениеКонстантыСервер("ТестируемаяБиблиотека");
	ПараметрыФормы = Новый Структура("AddInURL", ТестируемаяБиблиотека);
...
Процедура ПриОткрытии(Отказ)
	Отказ = Истина;
	ПутьКОбработке = ПолучитьЗначениеКонстантыСервер("ТестируемаяОбработка");
